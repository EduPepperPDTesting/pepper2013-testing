...
db.adjustment_time.find({'type':'external'}).forEach(function(ad){
  ...
  if(cur_item!=null){
    if (cur_item.adjustment_time == null){
      cur_item.adjustment_time = 0
    }
...
mysql -u$mysql_user -p$mysql_pwd -h$mysql_host -P$mysql_port $mysql_db <<EOF
select pepreg_training.id,type,pepreg_training.district_id,replace(description,'"','""') as description,subject,replace(pepreg_training.name,'"','""') as name,pepper_course,training_date,training_time_start,training_time_end,geo_location,classroom,credits,attendancel_id,if(allow_registration=1,'Yes','No')as allow_registration,if(allow_attendance=1,'Yes','No')as allow_attendance,if(allow_validation=1,'Yes','No')as allow_validation,max_registration,user_create_id,date_create,state_id,district.name as district,school.name as school from pepreg_training left join school on school.id=school_id,district where pepreg_training.district_id=district.id into outfile '/tmp/pepreg_training.csv' fields terminated by ',' optionally enclosed by '"' escaped by '' lines terminated by '\n';
select pepreg_student.training_id,student_id,student_status,instructor_id,student_credit from pepreg_student left join pepreg_instructor on pepreg_student.training_id=pepreg_instructor.training_id and student_id=instructor_id into outfile '/tmp/pepreg_student.csv' fields terminated by ',' optionally enclosed by '"' escaped by '' lines terminated by '\n';
EOF
...
$mongo3_path/mongoimport -d "$mongo3_db_reporting" -c "pepreg_training" --port $mongo3_port -f "training_id,type,district_id,description,subject,name,pepper_course,training_date,training_time_start,training_time_end,geo_location,classroom,credits,attendancel_id,allow_registration,allow_attendance,allow_validation,max_registration,user_create_id,date_create,state_id,district,school" --type=csv --file=/tmp/pepreg_training.csv
$mongo3_path/mongoimport -d "$mongo3_db_reporting" -c "pepreg_student" --port $mongo3_port -f "training_id,student_id,student_status,instructor_id,student_credit" --type=csv --file=/tmp/pepreg_student.csv
