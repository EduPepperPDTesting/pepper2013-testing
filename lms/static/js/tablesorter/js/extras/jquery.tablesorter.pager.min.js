				...
				if ( exception ) {
					...
					if (d instanceof jQuery) {
								...
								for ( i = 0; i < l; i++ ) {
									...
									for ( j = 0; j < d[i].length; j++ ) {
										...
										if(j == 0){
											...
											tds += /^\s*<td/.test(d[i][j]) ? $.trim(d[i][j]) : "<td><image style='width:16px;height:16px' src='/static/images/table-icon.jpg' class='aggregate_img'></image><span>" + d[i][j] + '<span></td>';
										}
									}
									tds += '</tr>';
								}
						...
						$(".aggregate_img").click(function(){
							val x = $(this).next().text();
							var y = $(this).parent().parent().find('td:first').text();
							$.get("${reverse('reporting_get_aggregate')}", {"report_id": "${report.id}","school_year":"${school_year}","row_head":y,"column_head":x}, function(datas){
