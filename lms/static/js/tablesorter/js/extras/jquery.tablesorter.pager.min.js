	...
	$.extend({ tablesorterPager: new function() {
			...
			if ( typeof p.ajaxProcessing === 'function' ) {
				...
				if ( exception ) {
					...
					if (d instanceof jQuery) {
						...
						if  ($('#row_header_type').length > 0 ){
							// row_header_type = $('#row_header_type').val();
							// if (row_header_type != 'time' || row_header_type != 'int'){
							...
							// }else{
								for ( i = 0; i < l; i++ ) {
									...
									for ( j = 0; j < d[i].length; j++ ) {
										// build tbody cells; watch for data containing HTML markup - see #434
										if(j == 0){
											tds += /^\s*<td/.test(d[i][j]) ? $.trim(d[i][j]) : "<td><image src='/static/images/icon_reporting.png' class='row_aggregate_img' onclick='row_graphable(this)'></image>&nbsp;&nbsp;" + d[i][j] + '</td>';
										}else{
											if(d[i][j] != 0){
												tds += /^\s*<td/.test(d[i][j]) ? $.trim(d[i][j]) : "<td><image style='width:16px;height:16px' src='/static/images/table-icon.jpg' class='aggregate_img' column="+ j +"></image>&nbsp;&nbsp;&nbsp;<span>" + d[i][j] + '<span></td>';
											}else{ 
												tds += /^\s*<td/.test(d[i][j]) ? $.trim(d[i][j]) : "<td><span style='margin-left:30px;'>" + d[i][j] + '<span></td>';
											}
						...
						$(".aggregate_img").click(function(){
							var x = $(this).attr('column')
							var y = $(this).parent().parent().find('td:first').text();
							get_ajax_get_aggregate(x,y)
							popWin("detail");
						})
					}
		...
		getAjax = function(table, p) {
			...
			if ( url !== '' ) {
                ...
                p.ajaxObject.type = p.method || 'get';
                p.ajaxObject.data = p.data;
		...
		updateCache = function(table) {
            ...
            $(table).triggerHandler( 'movingPage');
		...
		$this.construct = function(settings) {
			return this.each(function() {
                ...
                this.config.pager.reload=function(){
                    moveToPage(table, table.config.pager, true);
                    // updatePageDisplay(table,  table.config.page, true);
                }                
